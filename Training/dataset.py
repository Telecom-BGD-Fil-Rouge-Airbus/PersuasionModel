...
class SemEvalDataset(Dataset):
    def __init__(self, cfg, file_path, split='train', val_fold=0, transforms=None):
        self.data_root = cfg['dataset']['root']
        task = cfg['dataset']['task']
        self.transforms = transforms

        # load folds from json file
        with open(file_path + "data/folds.json", 'r') as f:
            folds = json.load(f)
        if split == 'train':
        ...
        if task == 3:
            label_file = os.path.join(file_path+self.data_root, '{}_set_task3'.format(self.split_name), '{}_set_task3.txt'.format(self.split_name))
            self.class_list = read_classes(file_path + 'techniques_list_task3.txt')
        elif task == 1:
            label_file = os.path.join(file_path+self.data_root, '{}_set_task1.txt'.format(self.split_name))
            self.class_list = read_classes(file_path + 'techniques_list_task1-2.txt')

        with open(label_file, 'r', encoding='utf8') as f:
            self.targets = json.load(f)
        for t in self.targets:
            t['path'] = os.path.join(file_path+self.data_root, '{}_set_task3'.format(self.split_name))

        if task == 3:
            label_file_dev = os.path.join(file_path+self.data_root, 'dev_set_task3_labeled', 'dev_set_task3_labeled.txt')
        elif task == 1:
            label_file_dev = os.path.join(file_path+self.data_root, 'dev_set_task1.txt')
        if os.path.isfile(label_file_dev) and self.split_name == 'training':
            with open(label_file_dev, 'r', encoding='utf8') as f:
                targets = json.load(f)
                for t in targets:
                    t['path'] = os.path.join(file_path+self.data_root, 'dev_set_task3_labeled')
...
class Collate:
    def __init__(self, file_path, config, classes):
        self.vocab_type = config['text-model']['name']
        self.data_root = file_path+config['dataset']['root']
