...
from format_checker import read_classes, check_format_task1_task3
from scorer import evaluate, _read_gold_and_pred
...
def main(opt):
    file_path = '/content/drive/MyDrive/Telecom/Airbus/Persusasion/MemePersuasionDetection/'
    ...
    collate_fn = Collate(file_path, cfg, classes)

    if opt.cross_validation:
        datasets = [SemEvalDataset(cfg, file_path, split='dev', transforms=test_transforms, val_fold=fold) for fold in range(len(checkpoints_files))]
        dataloaders = [DataLoader(dataset, batch_size=8, shuffle=False,
                                num_workers=2, collate_fn=collate_fn) for dataset in datasets]
    else:
        if opt.validate:
            dataset = SemEvalDataset(cfg, file_path, split='val', transforms=test_transforms, val_fold=opt.val_fold)
        elif opt.test:
            dataset = SemEvalDataset(cfg, file_path, split='test', transforms=test_transforms, val_fold=opt.val_fold)
        else:
            dataset = SemEvalDataset(cfg, file_path, split='dev', transforms=test_transforms)
    ...
    pred_labels, gold_labels = _read_gold_and_pred(predictions, dataloader.dataset.targets)
